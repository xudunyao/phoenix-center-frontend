...
const SearchForm = ({
  ...
  const { employmentStatus } = optionHooks.useEmploymentStatus();
  const [exportRecordDrawerVisible, setExportRecordDrawerVisible] = useState(false);
  const fields = generateFields({ tenantOption, typeOfWork, employmentStatus });
  ...
  const exportRecordSearchCriteria = fields.map((f) => {
    ...
    switch (f.name) {
      case 'tenantId':
        ...
        break;
      case 'status':
        render = (search) => employmentStatus.find((o) => o.value === search?.status)?.label;
        break;
      case 'typeOfWork':
