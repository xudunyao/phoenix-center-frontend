...
const SearchForm = ({
  ...
  const { signUpStatus } = optionHooks.useSignUpStatus();
  const [exportRecordDrawerVisible, setExportRecordDrawerVisible] = useState(false);
  const fields = generateFields({ tenantOption, typeOfWork, signUpStatus });
  ...
  const exportRecordSearchCriteria = fields.map((f) => {
    ...
    switch (f.name) {
      case 'tenantId':
        ...
        break;
      case 'status':
        render = (search) => signUpStatus.find((o) => o.value === search?.status)?.label;
        break;
      case 'typeOfWork':
