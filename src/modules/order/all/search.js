...
const SearchForm = ({
  ...
  const { allStatus } = optionHooks.useAllStatus();
  const fields = generateFields({ tenantOption, typeOfWork, allStatus });
  const exportRecordSearchCriteria = fields.map((f) => {
    ...
    switch (f.name) {
      case 'tenantId':
        ...
        break;
      case 'status':
        render = (search) => allStatus.find((o) => o.value === search?.status)?.label;
        break;
      case 'typeOfWork':
