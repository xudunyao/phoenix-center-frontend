...
const columns = [
  {
    ...
    render(text, record, index) {
      return record.entryDate ? moment(record.entryDate).format(datetimeFormat.date) : null;
    },
  }, {
    title: '离职日期',
    ...
    key: 'resignDate',
    render(text, record, index) {
      return record.resignDate ? moment(record.resignDate).format(datetimeFormat.date) : null;
    },
  }, {
    title: '报名日期',
...
const Employed = () => {
  ...
  const {
    ...
    data,
    refetch,
  } = useQuery(['orderEmployed', search], async () => {
    const res = await httpRequest.post('/order/joined/inquiry', search);
    if (res.code === 0) {
  ...
  const handleSearch = (searchval) => {
    const { entryDate, ...rest } = searchval;
    setSearchCriteria({
      ...searchCriteria,
      ...rest,
      entryDateStart: entryDate && moment(entryDate[0]).startOf('day').format('x'),
      entryDateEnd: entryDate && moment(entryDate[1]).endOf('day').format('x'),
  ...
  const exportHandle = async (val) => {
    ...
    try {
      const res = await httpRequest.post('/order/joined/export', params);
      if (res.code === 0) {
